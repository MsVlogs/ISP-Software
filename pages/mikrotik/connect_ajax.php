...
if (isset($_GET['mkid'])) {
    ...
    if ($mikrotikConnection) {
        ...
        foreach ($mikrotikConnection as $secret) {
            ...
            $actionbtn = '<button  data-status="' . $secret['disabled'] . '"   data-profile="' . $secret['profile'] . '" data-name="' . $secret['name'] . '" data-mkid="' . $_GET['mkidsecretunmatching'] . '" class="btn btn-xs btn-primary secretAddSoft"><iconify-icon icon="mdi:plus" class="text-xl"></iconify-icon></button>';
            $table .= '
            <tr>
            <td> ' . $sl++ . '</td>
            <td> ' . $secret['name'] . '</td>
